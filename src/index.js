...
progress.on("authFailure", (word) => {
  console.error(`❌ Authentication failed at "${word}".`);
});
progress.on("done", () => {
...
(async () => {
  let pass = 1;
  while (true) {
    // Load whatever remains in TEST.txt
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (entry.result.authenticated === false) {
        // read any previously failed words
        const existing = readFileSync(FAILED_FILE, "utf-8").split(/\r?\n/);
